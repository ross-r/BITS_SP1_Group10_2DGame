...
public class Camera {
	...
	public void project(Window window, Graphics g) {
		...
		if ( GameState.getInstance().isCameraOverridden() ) {
			...
			Matrix4f viewMatrix = setupViewMatrix(
				GameState.getInstance().getOverridenCameraX(),
				GameState.getInstance().getOverridenCameraY()
			);
			
			// Give NanoVG the matrix we would like to use for transformations.
			NanoVG.nvgTransform(g.vg(), 
					viewMatrix.m00(), viewMatrix.m01(), viewMatrix.m10(), 
					viewMatrix.m11(), viewMatrix.m30(), viewMatrix.m31());
			
		}
		else {
		
			// Compute the center of the players axis aligned bounding box and use this for the camera position.
			...
			// Make the camera position be centered around the player and in the center of the window.
