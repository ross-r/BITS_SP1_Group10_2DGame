...
public class Enemy extends GameObject {
	...
	public Enemy(Vector2f position) {
		...
		// Align the players bounding box so that x + w / 2 will be the center position
		// of the aabb.
		// (applies to y + h / 2 too)
		//
		// this is used so that the mouse position can be centered relative to the
		// players centered aabb.
		this.aabb.init(position.x - (Constants.PLAYER_BOUNDS / 2), position.y - (Constants.PLAYER_BOUNDS / 2),
				Constants.PLAYER_BOUNDS, Constants.PLAYER_BOUNDS);
	...
	// use this to register additional game objects, prevents exceptions being
	// thrown.
	public void initialize() {
		this.testProjectiles = new TestProjectile[Constants.MAX_PROJECTILES];

		for (int i = 0; i < Constants.MAX_PROJECTILES; i++) {
			this.testProjectiles[i] = (TestProjectile) ObjectList.getInstance()
					.createEntityObject(new TestProjectile());
		}
	}
...
//		texture.render(x, y, w, h, 255.f);
		...
		if (flSideMove < 0) { // LEFT
			// on ground
			if (bOnGround && (jumping == true)) {
				this.texture = this.texLeftJump3; // landing (jump)
				System.out.print("\nLanding");
				jumping = false;
			} else if (bOnGround && (jumping == false)) {
				// walk animation
				if (iAnimateTicks == 0) {
					this.texture = this.texLeftRun1; // run 1
					System.out.print("\nRun 1");
				}
				if (iAnimateTicks == 1) {
					this.texture = this.texLeftRun2; // run 2
					System.out.print("\nRun 2");
				}
			}
		}
		...
		if ((flJumpTime > 0.f) && !bOnGround) {
			this.texture = this.texLeftJump2; // mid-jump (jump)
			System.out.print("\nJump 2");
			// jump texture
			jumping = true;
		}
		// falling
		if ((flJumpTime <= 0.f) && !bOnGround) {
			this.texture = this.texLeftStand; // falling (stand)
			System.out.print("\nStand falling");
		...
		if (bIsAttacking) {
			if (iAnimateTicks == 0) {
				...
				// final attack
				this.texture = this.texLeftAtk4; // fin-atk (attack)
				System.out.print("\natk 4");
			}
		}

		if (flSideMove > 0) { // RIGHT
			// on ground
			if (bOnGround && (jumping == true)) {
				this.texture = this.texRightJump3; // landing (jump)
				jumping = false;
			} else if (bOnGround && (jumping == false)) {
				// walk animation
				if (iAnimateTicks == 0) {
					this.texture = this.texRightRun1; // run 1
				}
				if (iAnimateTicks == 1) {
					this.texture = this.texRightRun2; // run 2
				}
			}
		}
		// mid-jump
		if ((flJumpTime > 0.f) && !bOnGround) {
			this.texture = this.texRightJump2; // mid-jump (jump)
			// jump texture
			jumping = true;
		}
		// falling
		if ((flJumpTime <= 0.f) && !bOnGround) {
			this.texture = this.texRightStand; // falling (stand)
			// un-iterating walk texture
			jumping = true;
		}
		// attack
		if (bIsAttacking) {
			if (iAnimateTicks == 0) {
				// windup attack texture
				this.texture = this.texRightAtk1; // windup-atk (attack)
			} else if (iAnimateTicks == 1) {
				// final attack
				this.texture = this.texRightAtk4; // fin-atk (attack)
			}
		}
	...
	public void tick(Window w) {
		...
		iAnimateTicks = player.getiAnimateTicks();
		...
		// Calculate the distance to the player and don't attack if they're too far
		// away.
		...
		// Calculate the root of the magnitude which is the 'distance' between each
		// object.
		...
		if (iProjectileIndex >= 0 && iProjectileIndex < Constants.MAX_PROJECTILES) {
			...
			if (this.iProjectileCooldown == 0) {
				...
				Vector2f position = new Vector2f(flCenterX + (direction.x * Constants.PLAYER_BOUNDS),
	...
	private void jump(float flJumpStrength) {
		...
		if (this.bOnGround && this.flJumpTime <= 0.f) {
			...
			// We multiply by a fixed value here to keep the jump height consistent even
			// when we are moving faster.
	...
	private void onTakeDamage() {
		...
		// If we are going to die to this projectile, spawn some scrap on the ground as
		...
		if (this.iHealth <= 0) {
			...
			for (int i = 0; i < iRandScrapCount; i++) {
				...
				// Offset the x, y coordinates randomly to make it appear the scrap 'explodes'
				// out of the enemy.
